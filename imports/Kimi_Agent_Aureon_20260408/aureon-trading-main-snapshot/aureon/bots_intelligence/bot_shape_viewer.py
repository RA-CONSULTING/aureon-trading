...
def render_viewer():
    if not os.path.exists(SNAPSHOT_FILE):
        print(f"⚠️  Snapshot file '{SNAPSHOT_FILE}' not found. Run aureon_bot_shape_scanner.py first.")
        return
    ...
    if not shapes:
        print("⚠️  No shapes found in snapshot.")
        return

    print(f"🎨 Rendering 3D Bot Shapes for {len(shapes)} symbols...")
    ...
    print(f"✅ 3D View saved to: {os.path.abspath(OUTPUT_HTML)}")
