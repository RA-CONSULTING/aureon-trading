...
- Generated: `2026-05-15T17:50:00.867887+00:00`
...
| `ready_with_stale` | `self_improvement` | `proposal_patch_test_retest` | App Generation And Local Tool Builder | Keep generated code behind audit, benchmark, patch, retest, and explicit apply records. |
...
| `ready_with_stale` | `research` | `read_research_ingest_summarise` | Research, Search, And Vault Memory | Keep research outputs source-linked and visible in the vault/research tab. |
| `ready_with_stale` | `accounting` | `generate_manual_filing_support_documents` | Accounting Accounts And Evidence Builder | Refresh the accounting registry and show every missing evidence item before final-ready claims. |
...
| `ready_with_stale` | `self_improvement` | `safe_loop_with_apply_handoff` | Continuous Audit, Benchmark, Fix, Retest Loop | Keep observer running and use blockers as the next improvement queue. |
...
- `medium` `frontend_evolution.blocked_work_orders`: 40 frontend evolution work orders are blocked. Next: Resolve blocker status or create safe read-only status adapters.
