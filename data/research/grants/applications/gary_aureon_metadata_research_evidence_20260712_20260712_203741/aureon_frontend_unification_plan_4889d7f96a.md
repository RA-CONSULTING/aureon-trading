...
- Generated: `2026-05-22T18:17:00.026550+00:00`
...
| --- | ---: | ---: | --- | --- |
| `Overview` | 253 | 4 | none | observation |
| `Trading` | 2571 | 20 | none | live orders remain behind runtime trading gates |
| `Accounting` | 2043 | 6 | Accounting generated outputs need to be linked from the shell. | Companies House filing; HMRC submission; tax or penalty payment |
| `Research` | 2297 | 5 | none | observation |
| `SaaS Security` | 991 | 20 | Security blockers must be fixed before production promotion. | production deployment approval; third-party authorization scope signoff; 2 security blocker surface(s) visible in inventory. |
| `Self-Improvement` | 20 | 0 | none | code apply/restart approval when required |
| `Admin` | 979 | 20 | Security blockers must be fixed before production promotion. | credential entry; payment approval; official filing approval; 2 security blocker surface(s) visible in inventory. |
...
- `accounting_outputs`: `52` surfaces
- `auth_and_credentials`: `17` surfaces
- `dashboards`: `46` surfaces
- `trading_controls`: `52` surfaces
