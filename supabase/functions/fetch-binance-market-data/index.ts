...
serve(async (req) => {
  ...
  try {
    ...
    if (!apiKey || !apiSecret) {
      console.error('[fetch-binance-market-data] ❌ NO CREDENTIALS - Cannot fetch real data');
      return new Response(
        JSON.stringify({ 
          success: false, 
          error: 'NO_CREDENTIALS',
          message: 'Binance API credentials not configured. Cannot fetch live market data.',
          dataSource: 'NONE',
          demoMode: false,
        }),
        {
