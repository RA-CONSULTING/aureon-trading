...
// Simple encryption/decryption helpers (for demo - use proper encryption in production)
async function encryptApiSecret(secret: string, _key: string): Promise<string> {
  return btoa(secret); // Base64 encode (NOT SECURE - use proper encryption in production)
}

async function decryptApiSecret(encrypted: string, _key: string): Promise<string> {
