  ...
  const handleToggleRecording = async () => {
    if (isRecording) {
      ...
      try {
        ...
        const session = await startTranscriptionSession(stream, {
          onChunk: (text) => setChatInput(prev => prev + text),
          onEnd: () => {
            setIsRecording(false);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
             ...
             <div className="mt-8 pt-6 border-t border-gray-700">
                ...
                <ChartContainer title="QGITA Primary Indicators">
                    <AureonChart data={displayData.aureon.map(d => ({ ...d, time: d.time.toString() }))} />
                </ChartContainer>
            </div>
