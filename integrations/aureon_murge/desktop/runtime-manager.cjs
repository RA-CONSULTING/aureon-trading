...
const WEB_DIR = path.join(ROOT_DIR, 'web_app');
...
function spawnLogged(command, args, name, extraEnv = {}, cwd = ROOT_DIR) {
  ensureLogDir();
...
const SERVICE_DEFS = {
  ...
  runtime: {
    ...
    env: {
      ...
      MURGE_HOST_TERMINAL_ENABLED: process.env.MURGE_HOST_TERMINAL_ENABLED || '0',
      MURGE_SANDBOX_ENABLED: process.env.MURGE_SANDBOX_ENABLED || '0',
    },
  },
  aureon: {
    name: 'aureon',
...
class RuntimeManager {
  ...
  async ensureServices() {
    const skipAutoServers = process.env.FLAMEBORN_SKIP_AUTO_SERVERS === 'true';
    const status = {
      services: {},
      started: [],
      logs: {},
    ...
    if (skipAutoServers) {
      for (const key of ['aureon', 'web', 'runtime']) {
        status.services[key] = await this.inspectService(key);
      }
      return status;
    }
  ...
  async ensureService(name, forceRestart = false) {
    ...
    const reachable = await requestOk(def.healthUrl);
    if (!reachable) {
      const proc = spawnLogged(def.command, def.args, def.logName, def.env, def.cwd || ROOT_DIR);
